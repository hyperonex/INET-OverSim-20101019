...
httptReplyMessage * httptServerBase::handleRequestMessage( cMessage *msg )
{
	...
	if (method == RM_NONE || uri.empty())
	{
		...
		if ( res.size() != 3 )
		{
			...
			cerr<<"\n*********** "<<getParentModule()->getFullName()<< "Invalid request string: " << request->heading() <<"\n"<<endl;
	...
	if ( request->badRequest() )
	{
		...
		cerr<<"\n*********** "<<getParentModule()->getFullName()<< "Bad request - bad flag set. Message: " << request->getName() <<"\n"<<endl;
	...
	else
	{
		...
		cerr<<"\n*********** "<<getParentModule()->getFullName()<< "Unsupported request type " << httpMethodAsString(method) <<
				" for " << request->heading() <<"\n"<<endl;
