...
BitField::~BitField()
{
	...
	for (int i=0; i<numPieces_var; i++)
	{
		if (blockfield[i]){ delete blockfield[i]; blockfield[i] = NULL;}
		if (!init_from_msg)
			if (requested[i]){ delete requested[i];requested[i] = NULL;}
	}

	delete [] blockfield; blockfield = NULL;

	if (!init_from_msg)
		delete [] requested; requested = NULL;
...
void BitField::printRemainingPieces()
{
	...
	std::cerr<<"Remaining pieces"<<endl;
	std::cerr<<"----------------"<<endl;
	for (int i=0;i<numPieces();i++)
	{
		if (!bitfield[i])
...
void BitField::printRemainingBlocks()
{
	...
	std::cerr<<"Remaining blocks"<<endl;
	std::cerr<<"----------------"<<endl;
	for (int i=0;i<numPieces_var;i++)
	{
		for (int j=0;j<numBlocks_var;j++)
		{
			if (!blockfield[i][j])
				std::cerr<<"Block #"<<j<<" of piece "<<i<<endl;
		}
	}
}
