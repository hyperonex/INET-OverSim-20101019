...
bool PieceFreqEntry::operator<(const PieceFreqEntry &item) const {
	if ((frequency_var < item.frequency_var) || (frequency_var
			== item.frequency_var && index_var < item.index_var))
		return true;
...
void PieceFreqState::initializePieceFrequencies(int numOfPieces) {
	for (int i = 0; i < numOfPieces; i++) {
		pieceFreqVector.push_back(PieceFreqEntry(i, 0));
	}
...
void PieceFreqState::printPieceFrequencies() {
	...
	for (it = pieceFreqVector.begin(); it < pieceFreqVector.end(); it++) {
		ev << "\t\tIndex = " << (*it).getIndex() << " Frequency = "
				<< (*it).frequency() << endl;
	}
}
...
bool PeerEntry::operator<(const PeerEntry &item) const {
	...
	BTPeerWireClientHandlerBase* itemHandler =
			(BTPeerWireClientHandlerBase*) item.thread;
...
int PeerState::findPeer(string id) {
	for (unsigned int i = 0; i < peerVector.size(); i++)
		if (!strcmp(peerVector[i].getPeerID().c_str(), id.c_str()))
			return i;
...
void PeerState::removePeer(string id) {
	...
	if (index > -1) {
		/*if (peerVector.at(index).getPeerThread())
			delete peerVector.at(index).getPeerThread();*/
		peerVector.erase(peerVector.begin() + index);
	} else
		opp_error("BTUtils: Peer state management problem. Peer %s not found!",
				id.c_str());
}
...
void PeerState::print() {
	for (unsigned int i = 0; i < peerVector.size(); i++) {
		...
		BTPeerWireClientHandlerBase* threadBT =
				(BTPeerWireClientHandlerBase*) thread;
		cerr << "\tPeer entry #" << (i + 1) << " " << peerVector[i].getPeerID()
				<< ", state =  " << threadBT->getState() << ", Choked: "
				<< threadBT->peerChoking() << endl;
...
PeerEntry PeerState::getPeerEntry(const char* id) {
	for (unsigned int i = 0; i < peerVector.size(); i++)
		if (!strcmp(peerVector[i].getPeerID().c_str(), id))
			return peerVector[i];
...
RequestEntry::RequestEntry(int i, int b, int l, simtime_t t, const char* id,
	...
	response = (httptReplyMessage*) (httpReply);
	httpReply_var = response;
}
...
RequestEntry::RequestEntry(const RequestEntry& re) {
	...
	httpReply_var = re.httpReply_var;
}

RequestEntry::~RequestEntry() {
	//cerr << "deleting the httpReply..."<<endl;
	//if(httpReply_var) delete httpReply_var;
}
...
httptBaseMessage* RequestEntry::httpReplyMessage() {
	return httpReply_var;
}
void RequestEntry::setHttpReplyMessage(httptBaseMessage* httpReply) {
	httpReply_var = httpReply;
}
...
void RequestState::addRequest(RequestEntry re) {
	addRequest(re.getIndex(), re.begin(), re.length(), re.getTimestamp(),
			re.peerID(), re.httpReplyMessage());
}

void RequestState::addRequest(int index, int begin, int length, simtime_t t,
		const char* id, httptBaseMessage* httpReply) {
	RequestEntry entry(index, begin, length, t, id, httpReply);
	requestVector.push_back(entry);

	if ((checkSize()) && (requestVector.size() > maxSize()))
		opp_error("Exceeded maximum request queue size (maxSize = %d)",
				maxSize());
}
...
int RequestState::findRequest(int index, int begin) {
	for (unsigned int i = 0; i < requestVector.size(); i++)
		if ((requestVector[i].getIndex() == index) && (requestVector[i].begin()
				== begin))
			return i;
...
void RequestState::removeRequest(int index, int begin, bool silently) {
	...
	if (i > -1) {
		removeRequest(i);
	} else if ((!silently) && (size() > 0))
		opp_error(
				"BTUtils: Request state management problem. Request not found!");
}
...
void RequestState::print() {
	for (unsigned int i = 0; i < requestVector.size(); i++) {
		cerr << "\t\tRequest entry #" << (i + 1) << " ("
				<< requestVector[i].getIndex() << ","
				<< requestVector[i].begin() << ", "
				<< requestVector[i].getTimestamp() << ")" << endl;
		ev << "\t\tRequest entry #" << (i + 1) << " ("
				<< requestVector[i].getIndex() << ", "
				<< requestVector[i].begin() << ", "
				<< requestVector[i].getTimestamp() << ")" << endl;
	}
...
int RequestState::maxBlockRequested(int pieceIndex) {
	...
	for (unsigned int i = 0; i < requestVector.size(); i++) {
		if ((requestVector[i].getIndex() == pieceIndex)
				&& (requestVector[i].begin() > maxBlockRequested))
...
int RequestState::minBlockRequested(int pieceIndex) {
	...
	for (unsigned int i = 0; i < requestVector.size(); i++) {
		if ((requestVector[i].getIndex() == pieceIndex)
				&& (requestVector[i].begin() < minBlockRequested))
