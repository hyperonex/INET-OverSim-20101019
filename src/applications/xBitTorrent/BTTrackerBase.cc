...
void BTTrackerBase::initialize()
{
	...
	trackerId_var		= par("trackerId").stdstringValue();
...
int BTTrackerClientHandlerBase::processAnnounce(BTTrackerMsgAnnounce* amsg)
{
	...
	tpeer = new BTTrackerStructBase(IPvXAddress(sock->getRemoteAddress()), string(amsg->peerId()), amsg->peerPort(), string(amsg->key()), simTime(), (amsg->event() == A_COMPLETED) ? true : false);
...
void BTTrackerClientHandlerBase::sendResponse(int acode, BTTrackerMsgAnnounce* amsg)
{
	...
	// logging
	rmsg->setName("TrackerResponseMessage");
	BEV << "sending reply to client[address=" << sock->getRemoteAddress() << ", port=" << sock->getRemotePort() << " message kind= " << rmsg->getKind() << "]\n";
...
void BTTrackerClientHandlerBase::established()
{
	...
	// logging
	BEV << "connection with client[address=" << sock->getRemoteAddress() << ", port=" << sock->getRemotePort() << "] established\n";
	BEV << "starting session timer[" << hostModule()->sessionTimeout() << " secs] for client[address=" << sock->getRemoteAddress() << ", port=" << sock->getRemotePort() << "]\n";
...
{
	// logging
	BEV << "announce received from client[address=" << sock->getRemoteAddress() << ", port=" << sock->getRemotePort() << "]\n";
...
void BTTrackerClientHandlerBase::timerExpired(cMessage* msg)
{
	...
	// logging
	BEV << "session with client[address=" << sock->getRemoteAddress() << ", port=" << sock->getRemotePort() << "] expired\n";
	// perform close()
	sock->close();
}
...
void BTTrackerClientHandlerBase::peerClosed()
{
	...
	switch(state_var)
	{
		...
		case ESTABLISHED:
			// logging
			BEV << "session with client[address=" << sock->getRemoteAddress() << ", port=" << sock->getRemotePort() << "] died unexpectedly\n";
		...
		case TEARDOWN:
			// logging
			BEV << "session with client[address=" << sock->getRemoteAddress() << ", port=" << sock->getRemotePort() << "] ended\n";
