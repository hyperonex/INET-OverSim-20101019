...
class INET_API RequestEntry : public cObject
{
	protected:
		...
		httptBaseMessage* httpReply_var;

	public:
		RequestEntry();
		RequestEntry(int,int, int,simtime_t,const char*,httptBaseMessage*);
		...
		httptBaseMessage* httpReplyMessage();
		void setHttpReplyMessage(httptBaseMessage*);
		RequestEntry dup();
};
...
class INET_API RequestState
{
	...
	public:
		...
		void addRequest(int,int,int,simtime_t,const char*,httptBaseMessage*);
